...
#include <iostream>
#include <iomanip>
// Save all users to file by overwriting
void FileManager::saveUsers(const vector<User>& users) {
    // DO NOT use ios::app here if you are passing a vector
...
// Append a single transaction [cite: 126]
void FileManager::logTransaction(string tID, string type, double amount, string sender, string receiver, string date) {
   ...
   file << "Date: " << setw(12) << date 
     << " | " << setw(10) << sender  // This will now be the Name
     << " transferred Rs." << setw(8) << amount 
     << " to " << receiver           // This will now be the Name
     << " | ID: " << tID << endl;
             file.close();
    }
}